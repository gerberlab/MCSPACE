...
#################
melteddf2 <- fread("../ProcessedResultsV2_PE/melted_otu_table.csv")
...
bc1corrected <- data.table(rbind(bc1corrected7,bc1corrected8))
...
settocorrect <- settocorrect %>% select(-ReadNum)
...
CorrectedDataframe <- finaldf[, sum(Count), by = .(CorrectedParticleID,OTU,Sample)]
print("Unique Sample Names:")
print(unique(CorrectedDataframe$Sample))

# rename
names(CorrectedDataframe) <- c("Particle","OTU","Sample","Count")
CorrectedDataframe <- CorrectedDataframe %>%
  mutate(Particle = gsub('x', '_', Particle),
         Particle = gsub('R1', '', Particle),
         Sample = gsub('x', '_', Sample),
         Sample = gsub('_R1', '', Sample))

write.csv(CorrectedDataframe, "CorrectedDataFrame.csv", quote = F, row.names = F)
print("Writing Final data.table")
fwrite(CorrectedDataframe,"CorrectedDataFrame.csv")

#ADD METADATA & FIX NAMES


metadata <- read.csv("../misc/Maps_PRT02_combined_metadata.csv")

NSF_frame <- CorrectedDataframe %>% #mutate(Sample = gsub('x', '_', .$Sample))
    separate(Sample, into = c('prt', 'seqid', 'replicate'), sep = '_', remove = F) %>%
  #mutate(replicate = gsub('r', '', .$replicate)) %>%
  mutate(seqid = paste('PRT02-', seqid, sep = '')) %>% 
  left_join(., metadata, by = 'seqid') %>% select(-prt, -seqid) %>% 
  filter(!is.na(day)) #this removes JC's samples


#remove 
rows_with_na_days <- NSF_frame[is.na(NSF_frame$day), ]

write.csv(NSF_frame, '../ProcessedResultsV2_PE/20240428_40um_CorrectedDataframe.csv', row.names = F, quote = F)


#Now quick QC

fulldataset_allSamples <- read.csv("../ProcessedResultsV2_PE/CorrectedDataFrame.csv") %>% 
  ...
  filter(
    num_OTUs_particle > 1,
    total_particle_reads > 50, #changed from 100
...
#how mant particles?
read.csv("../ProcessedResults_PE/CorrectedDataFrame.csv") %>% select(Particle) %>% distinct() %>% nrow()
read.csv("../ProcessedResults_SE/CorrectedDataFrame.csv") %>% select(Particle) %>% distinct() %>% nrow()

#how many otus?
read.csv("../ProcessedResults_PE/CorrectedDataFrame.csv") %>% select(OTU) %>% distinct() %>% nrow()
read.csv("../ProcessedResults_SE/CorrectedDataFrame.csv") %>% select(OTU) %>% distinct() %>% nrow()


###unfiltered####

rbind((read.csv("../ProcessedResults_PE/CorrectedDataFrame.csv") %>% select(Particle, OTU) %>% distinct() %>%
  group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
  mutate(type = 'With Read2 (min 2 reads)')),
  (read.csv("../ProcessedResults_PE_MIN4/CorrectedDataFrame.csv") %>% select(Particle, OTU) %>% distinct() %>%
     group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
     mutate(type = 'With Read2 (min 4 reads)')),
(read.csv("../ProcessedResults_SE/CorrectedDataFrame.csv") %>% select(Particle, OTU) %>% distinct() %>%
  group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
  mutate(type = 'No Read2'))) %>% 
  ggplot(., aes(num_otus)) + geom_vline(xintercept = c(10,20), linetype = 'dashed', color = 'gray80') +
  geom_histogram(color = 'black', fill = 'dodgerblue', binwidth = 1) + 
  facet_wrap(~type) + theme_cowplot() +
  theme(strip.background = element_blank()) + scale_y_log10() + xlim(0,20) +
  labs(x = '# of OTUs in particle', y. = 'Count')

ggsave("../figs/Read2_OTUperParticle_unfiltered.png", width = 7, height = 2.5, bg = 'white')


##filtered

fulldataset_PE <- read.csv("../ProcessedResults_PE/CorrectedDataFrame.csv") %>% 
  #filter(OTU %in% otus.to.keep) %>%
  group_by(Particle) %>% 
  mutate(total_particle_reads = sum(Count),
         OTU_rel_abd = Count/sum(Count))  %>% 
...
fulldataset_SE <- read.csv("../ProcessedResults_SE/CorrectedDataFrame.csv") %>% 
  #filter(OTU %in% otus.to.keep) %>%
  group_by(Particle) %>% 
  mutate(total_particle_reads = sum(Count),
         OTU_rel_abd = Count/sum(Count))  %>% 
...
fulldataset_PE_min4 <- read.csv("../ProcessedResults_PE_MIN4/CorrectedDataFrame.csv") %>% 
  #filter(OTU %in% otus.to.keep) %>%
  group_by(Particle) %>% 
  mutate(total_particle_reads = sum(Count),
         OTU_rel_abd = Count/sum(Count))  %>% 
...
rbind((fulldataset_PE %>% select(Particle, OTU) %>% distinct() %>%
         group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
         mutate(type = 'With Read2 (min 2 reads)')),
      (fulldataset_PE_min4 %>% select(Particle, OTU) %>% distinct() %>%
         group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
         mutate(type = 'With Read2 (min 4 reads)')),
      (fulldataset_SE %>% select(Particle, OTU) %>% distinct() %>%
         group_by(Particle) %>% mutate(num_otus = n()) %>% select(Particle, num_otus) %>% distinct() %>% 
         mutate(type = 'No Read2'))) %>% 
  ggplot(., aes(num_otus)) + geom_vline(xintercept = c(10,20), linetype = 'dashed', color = 'gray80') +
    geom_histogram(color = 'black', fill = 'dodgerblue', binwidth = 1) + 
...
ggsave("../figs/Read2_OTUperParticle_filtered.png", width = 7, height = 2.5, bg = 'white')
