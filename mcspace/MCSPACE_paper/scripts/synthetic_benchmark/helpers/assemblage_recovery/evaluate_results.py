...
def evaluate_case(basepath, datapath, res, ds, npart, nreads, base_sample):
    ...
    for mod in models: #, klist in zip(models, ncomm_lists):
        ...
        res['number reads'].append(nreads)
        res['dataset'].append(ds)
        res['NMI'].append(nmi)
        res['community error'].append(comm_err)
        res['number clusters learned'].append(nk_learned)
        res['number clusters error'].append(nk_learned - 34)
    return res


def main(rootdir, outdir):
    rootpath = Path(rootdir)
    basepath = Path(outdir) / "assemblage_recovery" # path to model inference results
    ...
    datapath = Path(outdir) / "semisyn_data" / base_sample # path to semi-synthetic data
    for ds in dsets:
        ...
        for npart in npart_cases:
            results = evaluate_case(basepath, datapath, results, ds,  npart, "default", base_sample)
            print(f"\t {npart}")

        # vs read depth
        print("nreads cases")
        for nreads in nreads_cases:
            results = evaluate_case(basepath, datapath, results, ds, "default", nreads, base_sample)
            print(f"\t {nreads}")    
    
    # save results
    outpath = rootpath / "results" / "assemblage_recovery"
