...
#             nn.Softplus()
#         )
#         self.q_otu_mu_params = nn.Linear(self.hidden_dim, self.num_otus*self.num_communities) 
#         self.q_otu_var_params = nn.Linear(self.hidden_dim, self.num_otus*self.num_communities)
#         # self.z_otu_params = nn.Linear(self.hidden_dim, self.num_otus*self.num_communities) 
#         self.eps = None 
    
#     def sample_eps(self):
#         self.eps = torch.normal(0, 1, size=(self.num_communities,self.num_otus), device=self.device, requires_grad=False)

#     def compute_otu_z(self, normed_data, samples):
#         particle_otu_enc = self.q_otu_encode(normed_data) 
#         enc = particle_otu_enc.mean(dim=0) 
#         mu = self.q_otu_mu_params(enc).view(self.num_communities, self.num_otus)
#         var = torch.exp(self.q_otu_var_params(enc)).view(self.num_communities, self.num_otus)
#         z_otu = mu + torch.sqrt(var)*samples
#         return mu, var, z_otu
