...
def generate_dataset(beta, theta, num_particles, negbin_n, negbin_p, num_reads=None, pi_garb=None, gclust=None):
    ...
    if num_reads is None:
        counts, z = gen_reads_negbin(beta, theta_mix, num_particles, negbin_n, negbin_p)
    else:
        counts, z = gen_reads_fixed_rdepth(beta, theta_mix, num_particles, num_reads)
...
def gen_semisyn_data(base_sample):
    ...
    #* paths
    rootpath = Path("./")
    basepath = rootpath / "paper" / "semi_synthetic_data"
    ...
    for ds in dsets:
        ...
        for nk in nclust_cases:
            ...
            pickle_save(outpath / f"data_D{ds}_K{nk}_Pdefault_Rdefault_Gdefault_B{base_sample}.pkl", savedata)
        ...
        savedata = get_savedata(reads, assignments, newtheta, newbeta, pigarb_default)
        pickle_save(outpath / f"data_D{ds}_Kdefault_P{npart}_Rdefault_Gdefault_B{base_sample}.pkl", savedata)
        ...
        for nreads in nreads_cases:
            ...
            savedata = get_savedata(reads, assignments, newtheta, newbeta, pigarb_default)
            pickle_save(outpath / f"data_D{ds}_Kdefault_Pdefault_R{nreads}_Gdefault_B{base_sample}.pkl", savedata)
        ...
        for gpi in pgarb_cases:
            ...
            pickle_save(outpath / f"data_D{ds}_Kdefault_Pdefault_Rdefault_G{gpi}_B{base_sample}.pkl", savedata)
