...
def eval_cos_err_mcspace_samples(model, data, testcounts, dsreads, n_samples):
    ...
    # testcounts # LxO counts
    num_otus_per_particle = (testcounts>=1).sum(axis=1)
    for i in range(n_samples):
        print(f"sample {i} of {n_samples}")
        # _, theta, beta, pi_garb, gamma = model(data)
        _, theta, beta, gamma, pi = model(data)
        theta = theta.cpu().detach().clone().numpy() # TODO: make mixture theta???
        garb_dict = data['group_garbage_clusters']
        grp_key = list(garb_dict.keys())[0]
        garb = garb_dict[grp_key].cpu().detach().clone().numpy()
        if pi is None:
            theta_mix = theta
        else:
            pi_value = pi.item()
            theta_mix = (1.0 - pi_value)*theta + pi_value*garb[None,:] # KxO

        beta = beta[:,0,0].cpu().detach().clone().numpy() #! single time and subject 
        tassign = sample_assignments(theta_mix, beta, dsreads)
        s_reads = sample_reads(theta_mix, tassign, read_depths)
...
def evaluate_mcspace(case, fold, basepath, datapath):
    ...
    device = get_device()
    modelpath = basepath / "mcspace_runs" / case / f"Fold_F{fold}"
...
def main(rootdir, outdir):
    ...
    # save results
    outpath = rootpath / "results" / "cross_validation"
