...
run_case <- function(outpath, datapath, dset, num_clust, num_particles, nreads, gweight, nsubj, base_sample){
  print(paste("RUNNING dataset ", dset, " - num clusters", num_clust, " num particles: ", num_particles, " read depth: ", nreads, " gweight: ", gweight, " num subj: ", nsubj, sep=""))
  ...
  outfile <- paste("results_D",dset,"_K", num_clust, "_P",num_particles,"_R",nreads,"_G",gweight,"_B",base_sample,"_S",nsubj,".csv",sep="")
                  
  write.csv(distsum, file.path(outpath, outfile))
  print(paste("DONE dataset ", dset,  "- num particles: ", num_particles, " read depth: ", nreads, sep=""))
}
...
#for(bsample in list("Human", "Mouse")){
for(bsample in list("Mouse")){
  # data directory
  datapath <- file.path("ecosim_data", bsample)
  outpath <- file.path("ecosimR_results", bsample)
  dir.create(outpath, recursive=TRUE, showWarnings = FALSE)
  
  for(dset in 0:n_dsets){
    # vs num clusters
    for(nclust in nclust_cases){ 
      run_case(outpath, datapath, dset, nclust, "default", "default", "default", "default", bsample)
    }
    
    # vs num particles
    for(npart in npart_cases){
      run_case(outpath, datapath, dset, "default", npart, "default", "default", "default", bsample)
    }
    
    # vs read depth
    for(rdepth in rdepth_cases){
      run_case(outpath, datapath, dset, "default", "default", rdepth, "default", "default", bsample)
    }
    
    # vs contamination noise
    for(gweight in pgarb_cases){
      run_case(outpath, datapath, dset, "default", "default", "default", gweight, "default", bsample)
    }
    ...
    if (bsample == 'Mouse'){
      ...
      for(nsubj in nsubj_cases){
        run_case(outpath, datapath, dset, "default", "default", "default", "default", nsubj, bsample)
      }
    }
  }
}
